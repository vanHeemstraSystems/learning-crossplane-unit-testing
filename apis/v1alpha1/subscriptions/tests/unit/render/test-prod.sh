...
# test-prod.sh
# Unit test for production environment using crossplane render
...
echo "🧪 Testing production environment XR with crossplane render..."
...
if ! OUTPUT=$(crossplane render \
  "$REPO_ROOT/apis/v1alpha1/subscriptions/examples/xr-prod.yml" \
...
test_output "Subscription name is atlas-production" "subscriptionName: atlas-production"

# Test 5: Workload type is Production
test_output "Workload type is Production" "workload: Production"

# Test 6: Environment tag present
test_output "Environment tag present" "environment: prod"

# Test 7: Resource group has correct name
test_output "Resource group name is rg-atlas-prod-weu" "name: rg-atlas-prod-weu"
...
test_output "Cost center tag present" "cost-center: operations"

# Test 10: Project tag present
test_output "Project tag present" "project: atlas-idp"

# Test 11: Compliance tag present (prod-specific)
test_output "Compliance tag present" "compliance: required"
...
if [ $TESTS_FAILED -gt 0 ]; then
  ...
  echo ""
  echo -e "${GREEN}✅ All production environment tests passed!${NC}"
  exit 0
fi
